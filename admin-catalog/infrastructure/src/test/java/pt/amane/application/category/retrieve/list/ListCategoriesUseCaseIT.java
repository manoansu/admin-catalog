...
public class ListCategoriesUseCaseIT {
  ...
  void givenAValidTerm_whenTermsDoesntMatchesPrePersisted_shouldReturnEmptyPage() {
    ...
    final var aqQuery = new SearchQuery(expectedPage, expectedPerPage, expectedTerms, expectedSort, expectedDirection);

    final var actualResult = useCase.execute(aqQuery);
  ...
  void givenAValidIdSortAndDirection_whenCallsListCatigories_thenShouldReturnCategoriesOrdered(
    ...
    final var result = useCase.execute(aqQuery);

    final var actualResult = new Pagination<>(expectedPage, expectedPerPage, result.items().size(), result.items());
